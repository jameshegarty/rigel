...
PosToAddr = G.Module{ "PosToAddr", types.rv(types.Par(ar(u16,2))),
  function(loc)
    local i = G.PosSeq{{3,3},0}() -- inner loop from 0...2
    --print("ILOC",i.type,loc.type,i[0],loc[0])
...
ReadStencilDMA = G.Module{ "ReadStencilDMA", Handshake(types.Par(ar(u16,2))), SDF{1,9},
  function(loc)
    --print("LOC",loc.type,loc.rate)
    local locb = G.Broadcast{{3*3,1}}(loc) -- duplicate input (x,y) 9 times
    --print("OLOC",loc,locb)
    --print("LOCB",locb.type)
    local addrStream = G.Map{PosToAddr}(locb)
    --print("ADDRS",addrStream.type)
    local pxStream = G.AXIRead{"frame_128.raw",128*64,noc.read,types.Uint(8)}(addrStream)
    --local pxArr = G.Broadcast{1}(pxStream) -- convert to array with 1 element
    return G.Deser{3*3}(pxStream) -- Deserialize 9 reads into 9 element array
  end}

ConvTop = G.Module{ "ConvTop", SDF{1,30*14*9},
  function(i)
    --i = G.TriggerBroadcast{30*14}(i)
    local pos = G.Pos{{30,14}}(i)
    local posScaled = G.Mul{4}(pos) -- mult coords by 4
    local stencil = G.Map{ReadStencilDMA}(posScaled)
    local shifted = G.Rshift{3}(stencil)
