...
local a = G.SchedulableFunction{"a",types.int(32),
  function(i)
    ...
    local rhs = G.FloatRec{32}(G.Const{types.uint(48),7540982534034}(G.ValueToTrigger(i)))
    local res = G.AddF(lhs,rhs)
    return G.Float(res)
  end}
------------
...
harness{ outFile="float_add", fn=hsfn, inFile="trivial_64.raw", inSize={W,H}, outSize={W*4,H}, outP=8 }
