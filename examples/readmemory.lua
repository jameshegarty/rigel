...
local addr = RHLL.liftMath( function(pos) return (pos:index(0):index(0):addMSBs(8)+(pos:index(0):index(1)*f.constant(W))+f.constant(0):addMSBs(24)):addMSBs(6) end )(posseq)
...
local merge = RHLL.liftMath( function(pac) return (pac:index(0)+pac:index(1)):rshift(1):removeLSBs(1) end )(RS.fanIn{inp:selectStream(0),ramread:selectStream(0)})
...
harness{ outFile="readmemory", fn=fn, inFile="frame_128.raw", ramFile="frame_128.raw", inSize={W,H}, outSize={W,H}, harness=2, inP=1, outP=1, inType=ITYPE, outType=ITYPE, ramType=ITYPE }
