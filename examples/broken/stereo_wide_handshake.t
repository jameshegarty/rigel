...
function argmin()
  ...
  local indices = d.constant( "convKernel", range(SearchWindow), types.array2d( types.uint(8), SearchWindow ) ) -- uint8[SearchWindow]
...
function make()
  ...
  local left = d.apply( "LB", d.makeHandshake(d.stencilLinebuffer( types.uint(8), W, H, 1, -(SearchWindow+SADWidth)+2, 0, -SADWidth+1, 0 )), left)
  ...
  local merged = d.apply("merge", d.SoAtoAoSStateful( SearchWindow, 1, {STENCIL_TYPE,STENCIL_TYPE}, true ), d.tuple("mtup",{left,right})) -- {A[SADWidth,SADWidth],A[SADWidth,SADWidth]}[SearchWindow]
...
--  local OUT_TYPE = types.array2d(types.tuple{types.uint(8),types.uint(16)},4)
  ...
  harness.axi( "stereo_wide_handshake", hsfn,"stereo.raw",nil, nil,  TYPE,  4, W, H, OUT_TYPE, 8, W, H )
end
